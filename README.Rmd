...
```{r echo=FALSE, fig.align='center', fig.cap='SIR simulation run once, N = 10,000 and dt = 0.01'}
...
This time the model is run a hundred times for 10,000 iterations with a time step of 0.01 using the same code.

```{r echo=FALSE, fig.align='center', fig.cap='SIR simulation run 100 times, N = 10,000 and dt = 0.01'}
options(warn = - 1) 
dt <- 0.01
count <- 1:10
...
for (j in count)
  {
    ...
    sir_col <- c("#8c8cd9", "#cc0044", "#999966")
    par(mar = c(4.1, 5.1, 0.5, 0.5), las = 1, new=TRUE)
    matplot(results$time[1:10000], cbind(results$S[1:10000], results$I[1:10000], results$R[1:10000]), type = "l", col = sir_col, lty = 1, xlab="time", ylab="S, I, R", main = "Eruptions of Old Faithful")
    legend("topright", inset = 0.05, lwd = 1, col = sir_col, legend = c("S", "I", "R"))
}
```
